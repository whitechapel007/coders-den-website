...
export function QuizResults({ quizData, quiz, onContinue, onRetake }: QuizResultsProps) {
  ...
  const getRecommendedPath = (score: number) => {
    if (score >= 80) {
      return {
        ...
        description: 'You\'re ready for advanced topics and can help mentor others.',
        suggestions: [
      ...
      return {
        ...
        description: 'Build on your foundation with practical projects and mentorship.',
        suggestions: [
        ...
        <CardContent>
          <div className="space-y-6">
            {results.map((result, index) => (
              <div key={result.question.id} className="border-b border-border pb-6 last:border-b-0">
                <div className="flex items-start space-x-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium mb-2">
                      Question {index + 1}: {result.question.question.split('```')[0].trim()}
                    </h4>
                    <div className="text-sm text-muted-foreground mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {result.question.techStack} • {result.question.difficulty}
                      </span>
                    </div>
                    {!result.isCorrect && (
